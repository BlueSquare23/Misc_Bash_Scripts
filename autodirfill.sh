...
function mndir(){
	...
	do
		...
		#It checks to make sure the function is only recursing to the specified debth --------------------------v
		...
		then	#I had to do an expr subtraction on this ^ because my username has two number in it ----^ which otherwise would be counted in this egrep <<< pwd count
			...
			cd $x
			mkdir {0..3}
			#Then the function calls itself and start the loop over filling in the empty directories to the specfied debth
			mndir 
...
#Because of the exponential nature of this script it is not reccomended to run this with any more than a debth of 5.
#At a debth of 5, making 6 dirs (i.e. {0..5}) in each sub dir, you will create 55986 directories or 6^6 + 6^5 + 6^4 + 6^3 + 6^2 + 6.
